...
if [ "$installtomcat" = "y" ]; then
  ...
  sudo curl -s -o $CATALINA_HOME/conf/server.xml $BASE_DOWNLOAD/tomcat/server.xml
  sudo curl -s -o $CATALINA_HOME/conf/catalina.properties $BASE_DOWNLOAD/tomcat/catalina.properties
  sudo curl -s -o $CATALINA_HOME/conf/tomcat-users.xml $BASE_DOWNLOAD/tomcat/tomcat-users.xml
  sudo curl -s -o $CATALINA_HOME/conf/context.xml $BASE_DOWNLOAD/tomcat/context.xml
  if [ "$ISON1604" = "y" ]; then
    sudo curl  -s -o /etc/systemd/system/alfresco.service $BASE_DOWNLOAD/tomcat/alfresco.service
  ...
  installpg=y
  if [ "$installpg" = "y" ]; then
	curl -s -O $JDBCPOSTGRESURL/$JDBCPOSTGRES
  ...
  if [ "$installmy" = "y" ]; then
	...
	curl -s -L -O $JDBCMYSQLURL/$JDBCMYSQL
	tar xf $JDBCMYSQL
...
EOF
  ...
  #sudo curl -s -o /etc/nginx/conf.d/alfresco.conf $BASE_DOWNLOAD/nginx/alfresco.conf
  #sudo curl -s -o /etc/nginx/conf.d/alfresco.conf.ssl $BASE_DOWNLOAD/nginx/alfresco.conf.ssl 
  #sudo curl -s -o /etc/nginx/conf.d/basic-settings.conf $BASE_DOWNLOAD/nginx/basic-settings.conf
  ...
  if [ ! -f "$ALF_HOME/www/maintenance.html" ]; then
    ...
    sudo curl -s -o $ALF_HOME/www/maintenance.html $BASE_DOWNLOAD/nginx/maintenance.html
  fi
...
# Always add the addons dir and scripts
  ...
  if [ ! -f "$ALF_HOME/addons/alfresco-mmt.jar" ]; then
    sudo curl -s -o $ALF_HOME/addons/alfresco-mmt.jar $ALFMMTJAR
  fi
  ...
  if [ ! -f "$ALF_HOME/bin/alfresco-pdf-renderer" ]; then
    ...
    sudo curl -s -o $TMP_INSTALL/alfresco-pdf-renderer.tgz $ALFRESCO_PDF_RENDERER
  ...
  if [ ! -f "$ALF_HOME/scripts/mariadb.sh" ]; then
    ...
    sudo curl -s -o $ALF_HOME/scripts/mariadb.sh $BASE_DOWNLOAD/scripts/mariadb.sh
  fi
  ...
  if [ ! -f "$ALF_HOME/scripts/mysql.sh" ]; then
    ...
    sudo curl -s -o $ALF_HOME/scripts/mysql.sh $BASE_DOWNLOAD/scripts/mysql.sh
  fi
  ...
  if [ ! -f "$ALF_HOME/scripts/createssl.sh" ]; then
    ...
    sudo curl -s -o $ALF_HOME/scripts/createssl.sh $BASE_DOWNLOAD/scripts/createssl.sh
  fi
  if [ ! -f "$ALF_HOME/scripts/libreoffice.sh" ]; then
    echo "Downloading libreoffice.sh script..."
    sudo curl -s -o $ALF_HOME/scripts/libreoffice.sh $BASE_DOWNLOAD/scripts/libreoffice.sh
  ...
  if [ ! -f "$ALF_HOME/scripts/iptables.sh" ]; then
    ...
    sudo curl -s -o $ALF_HOME/scripts/iptables.sh $BASE_DOWNLOAD/scripts/iptables.sh
  fi
  if [ ! -f "$ALF_HOME/scripts/alfresco-iptables.conf" ]; then
    echo "Downloading alfresco-iptables.conf upstart script..."
    sudo curl -s -o $ALF_HOME/scripts/alfresco-iptables.conf $BASE_DOWNLOAD/scripts/alfresco-iptables.conf
  fi
  if [ ! -f "$ALF_HOME/scripts/ams.sh" ]; then
    echo "Downloading maintenance shutdown script..."
    sudo curl -s -o $ALF_HOME/scripts/ams.sh $BASE_DOWNLOAD/scripts/ams.sh
  fi
  ...
  if [ ! -f " $ALF_DATA_HOME/keystore/ssl.keystore" ]; then
    ...
    sudo curl -s -o $ALF_DATA_HOME/keystore/browser.p12 $KEYSTOREBASE/browser.p12
    sudo curl -s -o $ALF_DATA_HOME/keystore/generate_keystores.sh $KEYSTOREBASE/generate_keystores.sh
    sudo curl -s -o $ALF_DATA_HOME/keystore/keystore $KEYSTOREBASE/keystore
    sudo curl -s -o $ALF_DATA_HOME/keystore/keystore-passwords.properties $KEYSTOREBASE/keystore-passwords.properties
    sudo curl -s -o $ALF_DATA_HOME/keystore/ssl-keystore-passwords.properties $KEYSTOREBASE/ssl-keystore-passwords.properties
    sudo curl -s -o $ALF_DATA_HOME/keystore/ssl-truststore-passwords.properties $KEYSTOREBASE/ssl-truststore-passwords.properties
    sudo curl -s -o $ALF_DATA_HOME/keystore/ssl.keystore $KEYSTOREBASE/ssl.keystore
    sudo curl -s -o $ALF_DATA_HOME/keystore/ssl.truststore $KEYSTOREBASE/ssl.truststore
  fi
...
if [ "$installwar" = "y" ]; then
  ...
  sudo curl -s -o $ALF_HOME/addons/war/alfresco.war $ALFREPOWAR
  echo

  # Add default alfresco and share modules classloader config files
  sudo curl -s -o $CATALINA_HOME/conf/Catalina/localhost/alfresco.xml $BASE_DOWNLOAD/tomcat/alfresco.xml
...
if [ "$installsharewar" = "y" ]; then
  ...
  sudo curl -s -o $ALF_HOME/addons/war/share.war $ALFSHAREWAR

  # Add default alfresco and share modules classloader config files
  sudo curl -s -o $CATALINA_HOME/conf/Catalina/localhost/share.xml $BASE_DOWNLOAD/tomcat/share.xml
...
if [ "$installbart" = "y" ]; then
 ...
 sudo curl -s -o $TMP_INSTALL/$BART_PROPERTIES $BASE_BART_DOWNLOAD$BART_PROPERTIES
 sudo curl -s -o $TMP_INSTALL/$BART_EXECUTE $BASE_BART_DOWNLOAD$BART_EXECUTE
